...
public class TimeEffect {
	...
	public TimeEffect(HyperConomy hc, TimeEffectType type, String name, String economy, double value, int seconds,
			double increment, int timeRemaining) {
		this.hc = hc;
		this.type = type;
		this.name = name;
	...
	public void subtractTime(int seconds) {
		...
		if (timeRemaining <= 0) {
			...
			if (this.timeRemaining < 0)
				this.timeRemaining = 0; // in case subtract interval is larger than time effect seconds
		}
		HashMap<String, String> values = new HashMap<String, String>();
		values.put("TIME_REMAINING", timeRemaining + "");
