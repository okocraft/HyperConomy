...
public class DatabaseUpdater {
	...
	/*
	 * public double getCurrentDatabaseVersion() { return currentDbVersion; }
	 */
	...
	public boolean updateTables(QueryResult qr) {
		...
		if (qr.next()) {
			...
			if (currentDbVersion == 1.34) {// adds ability for player shops to behave like server shops
				...
				Field f = t.generateField("USE_ECONOMY_STOCK", FieldType.VARCHAR);
				f.setFieldSize(100);
				f.setNotNull();
				f.setDefault("1");
			...
			if (currentDbVersion == 1.35) {// converts to new object data storage format and moves categories to
											// database
				BukkitConnector bc = (BukkitConnector) hc.getMC();
				QueryResult result = sr
						.select("SELECT NAME, TYPE, DATA FROM hyperconomy_objects WHERE ECONOMY = 'default'");
				while (result.next()) {
					...
					if (type.equalsIgnoreCase("ITEM")) {
						...
						sw.addToQueue("UPDATE hyperconomy_objects SET DATA = '" + n.serialize() + "' WHERE NAME = '"
								+ name + "'");
				...
				for (Map.Entry<String, String> entry : data.entrySet()) {
					sw.addToQueue("UPDATE hyperconomy_objects SET CATEGORIES = '" + entry.getValue() + "'"
							+ " WHERE (NAME = '" + entry.getKey() + "' OR DISPLAY_NAME = '" + entry.getKey()
							+ "' OR ALIASES LIKE '%" + entry.getKey() + ",%')");
				}
				new Backup(hc);
				hc.getYamlHandler().unRegisterFileConfiguration("categories");
				hc.getFileTools()
						.deleteFile(hc.getSimpleDataLib().getStoragePath() + File.separator + "categories.yml");
				setDBVersion(1.36);
				sw.writeSyncQueue();
			}
			if (currentDbVersion == 1.36) {// moves composites to objects table
				...
				Table t = hc.getSQLManager().getTable("hyperconomy_objects");
				Field f = t.generateField("COMPONENTS", FieldType.VARCHAR);
				f.setFieldSize(1000);
				f.setNotNull();
				f.setDefault("");
				...
				while (result.next()) {
					...
					QueryResult result2 = sr
							.select("SELECT COMPONENTS FROM hyperconomy_composites WHERE NAME = '" + name + "'");
					while (result2.next()) {
						String components = result2.getString("COMPONENTS");
						sw.addToQueue("UPDATE hyperconomy_objects SET COMPONENTS = '" + components + "' WHERE NAME = '"
								+ name + "' AND ECONOMY = '" + economy + "'");
					}
				}
			...
			if (currentDbVersion == 1.37) {// adds time effects table
				...
				Field f = t.addField("TYPE", FieldType.VARCHAR);
				f.setFieldSize(255);
				f.setNotNull();
				compositeKey.add(f);
				f = t.addField("NAME", FieldType.VARCHAR);
				f.setFieldSize(255);
				f.setNotNull();
				compositeKey.add(f);
				f = t.addField("ECONOMY", FieldType.VARCHAR);
				f.setFieldSize(255);
				f.setNotNull();
				compositeKey.add(f);
				...
				f.setNotNull();
				f.setDefault("0");
				f = t.addField("SECONDS", FieldType.INTEGER);
				f.setNotNull();
				f.setDefault("0");
				f = t.addField("INCREMENT", FieldType.DOUBLE);
				f.setNotNull();
				f.setDefault("0");
				f = t.addField("TIME_REMAINING", FieldType.INTEGER);
				f.setNotNull();
				f.setDefault("0");
			...
			if (currentDbVersion == 1.38) {// adds object versioning
				Table t = hc.getSQLManager().getTable("hyperconomy_objects");
				Field f = t.generateField("VERSION", FieldType.DOUBLE);
				f.setNotNull();
				f.setDefault("1");
			...
			if (currentDbVersion == 1.39) {// adds chest shops, moves item data
				...
				Field f = t.addField("ID", FieldType.INTEGER);
				f.setPrimaryKey();
				f.setAutoIncrement();
				...
				f = t.addField("ID", FieldType.VARCHAR);
				f.setFieldSize(255);
				f.setNotNull();
				f.setPrimaryKey();
				f = t.addField("WORLD", FieldType.VARCHAR);
				f.setFieldSize(100);
				f.setNotNull();
				f = t.addField("X", FieldType.INTEGER);
				f.setNotNull();
				f = t.addField("Y", FieldType.INTEGER);
				f.setNotNull();
				f = t.addField("Z", FieldType.INTEGER);
				f.setNotNull();
				f = t.addField("OWNER", FieldType.VARCHAR);
				f.setFieldSize(255);
				f.setNotNull();
				f = t.addField("PRICE_INCREMENT", FieldType.VARCHAR);
				f.setFieldSize(100);
				f.setNotNull();
				t.save();
				t = hc.getSQLManager().addTable("hyperconomy_chest_shop_items");
				ArrayList<Field> compositeKey = new ArrayList<Field>();
				f = t.addField("CHEST_ID", FieldType.VARCHAR);
				f.setFieldSize(255);
				f.setNotNull();
				compositeKey.add(f);
				...
				f.setNotNull();
				compositeKey.add(f);
				f = t.addField("BUY_PRICE", FieldType.DOUBLE);
				f.setNotNull();
				f = t.addField("SELL_PRICE", FieldType.DOUBLE);
				f.setNotNull();
				f = t.addField("TYPE", FieldType.VARCHAR);
				f.setFieldSize(40);
				f.setNotNull();
				t.setCompositeKey(compositeKey);
				t.save();
		...
		if (requiredDbVersion != currentDbVersion) {
			hc.getMC().logSevere("[HyperConomy]Your database (version " + currentDbVersion
					+ ") is not compatible with this version of HyperConomy.");
			hc.getMC().logSevere(
					"[HyperConomy]Please read the upgrading page in the HyperConomy wiki for more information.");
	...
	public void createTables() {
		...
		Field f = t.addField("SETTING", FieldType.VARCHAR);
		f.setFieldSize(100);
		f.setNotNull();
		f.setPrimaryKey();
		...
		f.setNotNull();
		compositeKey.add(f);
		f = t.addField("DISPLAY_NAME", FieldType.VARCHAR);
		f.setFieldSize(255);
		f = t.addField("ALIASES", FieldType.VARCHAR);
		f.setFieldSize(1000);
		...
		f = t.addField("MAXSTOCK", FieldType.DOUBLE);
		f.setNotNull();
		f.setDefault("1000000");
		f = t.addField("COMPONENTS", FieldType.VARCHAR);
		f.setFieldSize(1000);
		f.setNotNull();
		f.setDefault("");
		f = t.addField("DATA_ID", FieldType.INTEGER);
		f = t.addField("VERSION", FieldType.DOUBLE);
		f.setNotNull();
		f.setDefault("1");
		t.setCompositeKey(compositeKey);

		t = hc.getSQLManager().addTable("hyperconomy_players");
		f = t.addField("ID", FieldType.INTEGER);
		f.setNotNull();
		f.setPrimaryKey();
		f.setAutoIncrement();
		f = t.addField("NAME", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setUnique();
		f = t.addField("UUID", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setUnique();
		...
		f.setNotNull();
		f.setDefault("0");
		f = t.addField("X", FieldType.DOUBLE);
		f.setNotNull();
		f.setDefault("0");
		f = t.addField("Y", FieldType.DOUBLE);
		f.setNotNull();
		f.setDefault("0");
		f = t.addField("Z", FieldType.DOUBLE);
		f.setNotNull();
		f.setDefault("0");
		f = t.addField("WORLD", FieldType.TINYTEXT);
		f.setNotNull();
		f = t.addField("HASH", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setNotNull();
		f.setDefault("");
		f = t.addField("SALT", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setNotNull();
		f.setDefault("");
		...
		f = t.addField("SHOP", FieldType.VARCHAR);
		f.setFieldSize(100);
		f.setNotNull();
		compositeKey.add(f);
		f = t.addField("HYPEROBJECT", FieldType.VARCHAR);
		f.setFieldSize(100);
		f.setNotNull();
		compositeKey.add(f);
		f = t.addField("QUANTITY", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("SELL_PRICE", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("BUY_PRICE", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("MAX_STOCK", FieldType.INTEGER);
		f.setNotNull();
		f.setDefault("1000000");
		f = t.addField("STATUS", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setNotNull();
		t.setCompositeKey(compositeKey);
		...
		f = t.addField("SHOP", FieldType.VARCHAR);
		f.setFieldSize(255);
		f = t.addField("TRADE_AMOUNT", FieldType.INTEGER);
		f.setNotNull();
		f = t.addField("X", FieldType.DOUBLE);
		f.setNotNull();
		f.setDefault("0");
		...
		f.setNotNull();
		f.setDefault("0");
		f = t.addField("OWNERS", FieldType.VARCHAR);
		f.setFieldSize(255);
		f = t.addField("MEMBERS", FieldType.VARCHAR);
		f.setFieldSize(255);
		...
		f.setFieldSize(255);
		f.setNotNull();
		f = t.addField("USE_ECONOMY_STOCK", FieldType.VARCHAR);
		f.setFieldSize(100);
		f.setNotNull();
		f.setDefault("1");
		f = t.addField("MESSAGE", FieldType.TEXT);
		f.setNotNull();
		f = t.addField("BANNED_OBJECTS", FieldType.TEXT);
		f.setNotNull();
		f = t.addField("ALLOWED_PLAYERS", FieldType.TEXT);
		f.setNotNull();
		f = t.addField("P1X", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("P1Y", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("P1Z", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("P2X", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("P2Y", FieldType.DOUBLE);
		f.setNotNull();
		f = t.addField("P2Z", FieldType.DOUBLE);
		f.setNotNull();
		...
		f.setFieldSize(255);
		f.setNotNull();
		f = t.addField("MULTIPLIER", FieldType.INTEGER);
		f.setNotNull();
		f = t.addField("ECONOMY", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setNotNull();
		f = t.addField("ECLASS", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setNotNull();
		t.setCompositeKey(compositeKey);
		...
		f.setNotNull();
		f.setPrimaryKey();
		f = t.addField("HYPERACCOUNT", FieldType.VARCHAR);
		f.setFieldSize(255);
		f.setNotNull();
		...
		sw.addToQueue("INSERT INTO hyperconomy_settings (SETTING, VALUE, TIME) VALUES ('version', '" + requiredDbVersion
				+ "', NOW() )");
