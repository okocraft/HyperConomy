...
public class HyperConomy implements HyperEventListener, SDLEventListener {
	...
	public void handleSDLEvent(SDLEvent event) {
		if (event instanceof LogEvent) {
			...
			if (levent.getLevel() == LogLevel.SEVERE || levent.getLevel() == LogLevel.ERROR)
				mc.logSevere(levent.getMessage());
			if (levent.getLevel() == LogLevel.INFO)
				mc.logInfo(levent.getMessage());
	...
	private void preEnable() {
		...
		dMode.syncDebugConsoleMessage(
				"HyperConomy loaded with Debug Mode enabled.  Configuration files created and loaded.");
