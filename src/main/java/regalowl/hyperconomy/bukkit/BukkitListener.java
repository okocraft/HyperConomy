...
public class BukkitListener implements Listener {
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		if (hp == null)
			hp = hc.getHyperPlayerManager().getHyperPlayer(event.getPlayer().getName());
	...
	public void onPlayerInteractEvent(PlayerInteractEvent event) {
		...
		HyperPlayer hp = null;
		if (!minimal)
			hp = bc.getBukkitCommon().getPlayer(event.getPlayer());
		...
		if (minimal)
			if (bc.isTransactionSign(block.getLocation()))
				event.setCancelled(true);
		if (hpie.isCancelled())
			event.setCancelled(true);
	}
	...
	public void onChestShopInventoryClickEvent(InventoryClickEvent icevent) {
		...
		ChestShopClickEvent event = new ChestShopClickEvent(clicker, openShop, icevent.getRawSlot(),
				icevent.getAction().toString().toUpperCase(), clickedStack.getAmount(), clickedStack,
				cursorStack.getAmount(), cursorStack);
	...
	public void onChestShopInventoryOpenEvent(InventoryOpenEvent ioevent) {
		...
		/*
		 * if (cs == null) { BukkitCommon common = bc.getBukkitCommon(); //if
		 * (!bc.getBukkitCommon().isChestShopChest(chestLoc)) return; Block b =
		 * common.getBlock(chestLoc); if (b == null) return; if (!(b.getState()
		 * instanceof Chest)) return; Chest chest = (Chest) b.getState(); HLocation sl =
		 * common.getLocation(chest.getLocation()); sl.setY(sl.getY() + 1); Block sb =
		 * common.getBlock(sl); if (sb == null) return; if (!(sb.getState() instanceof
		 * Sign)) return; Sign s = (Sign) sb.getState();
		 * 
		 * String line1 = ChatColor.stripColor(s.getLine(0)).trim(); if (!hc.enabled()
		 * && line1.equalsIgnoreCase("ChestShop")) { ioevent.setCancelled(true); return;
		 * } String line2 = ChatColor.stripColor(s.getLine(1)).trim();
		 * 
		 * if (!line2.equalsIgnoreCase("[Trade]") && !line2.equalsIgnoreCase("[Buy]") &&
		 * !line2.equalsIgnoreCase("[Sell]")) return; String line34 =
		 * hc.getMC().removeColor(s.getLine(2)).trim() +
		 * hc.getMC().removeColor(s.getLine(3)).trim(); if
		 * (!hc.getDataManager().accountExists(line34)) return; HyperAccount owner =
		 * hc.getDataManager().getAccount(line34); s.setLine(1, ""); s.setLine(2, "");
		 * s.setLine(3, ""); s.update(); cs =
		 * hc.getChestShopHandler().addNewChestShop(chestLoc, owner); }
		 */
