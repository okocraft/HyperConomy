...
public class BukkitCommon {
	...
	protected static final BlockFace[] planeFaces = { BlockFace.EAST, BlockFace.WEST, BlockFace.NORTH,
			BlockFace.SOUTH };
	protected static final BlockFace[] allFaces = { BlockFace.EAST, BlockFace.WEST, BlockFace.NORTH, BlockFace.SOUTH,
	...
	protected boolean isChestShopSignBlock(HLocation l) {
		...
		if (sign == null)
			return false;
		if (!isChestShopSign(sign.getLocation()))
			return false;
		return true;
	}
	...
	protected boolean isPartOfChestShop(HLocation l) {
		if (isChestShopChest(l))
			return true;
		if (isChestShopSign(l))
			return true;
		if (isChestShopSignBlock(l))
			return true;
		return false;
	}
	...
	protected void setInventory(HInventory inventory) {
		...
		if (currentInventory.size() != newInventory.size())
			return;
		for (int i = 0; i < newInventory.size(); i++) {
			if (newInventory.get(i).equals(currentInventory.get(i)))
				continue;
	...
	protected void setBukkitInventory(Inventory bukkitInventory, HInventory inventory) {
		if (inventory == null || bukkitInventory == null)
			return;
		if (bukkitInventory.getSize() != inventory.getSize())
			return;
		for (int i = 0; i < inventory.getSize(); i++) {
			if (inventory.getItem(i).isBlank())
				continue;
	...
	public HItemStack getSerializableItemStack(ItemStack s) {
		...
		HItemStack sis = new HItemStack(
				hc, new HItemMeta("", new ArrayList<String>(), new ArrayList<HEnchantment>(),
						new ArrayList<HItemFlag>(), false, 0),
				material, durability, data, amount, maxStackSize, maxDurability);
		if (isBlank)
			sis.setBlank();
		if (s.hasItemMeta()) {
			...
			if (im instanceof EnchantmentStorageMeta) {
				...
				itemMeta = new HEnchantmentStorageMeta(displayName, lore, itemFlags, unbreakable, repairCost,
						storedEnchantments);
			} else if (im instanceof BookMeta) {
				BookMeta sItemMeta = (BookMeta) im;
				ArrayList<String> pages = new ArrayList<String>();
				if (sItemMeta.getPages() != null)
					pages.addAll(sItemMeta.getPages());
				itemMeta = new HBookMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						sItemMeta.getAuthor(), pages, sItemMeta.getTitle());
				...
				if (fe != null) {
					...
					sfe = new HFireworkEffect(colors, fadeColors, fe.getType().toString(), fe.hasFlicker(),
							fe.hasTrail());
				}
				itemMeta = new HFireworkEffectMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						sfe);
				...
				if (sItemMeta.getEffects() != null) {
					for (FireworkEffect fe : sItemMeta.getEffects()) {
						...
						fireworkEffects.add(new HFireworkEffect(colors, fadeColors, fe.getType().toString(),
								fe.hasFlicker(), fe.hasTrail()));
					}
				}
				itemMeta = new HFireworkMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						fireworkEffects, sItemMeta.getPower());
			} else if (im instanceof LeatherArmorMeta) {
				LeatherArmorMeta sItemMeta = (LeatherArmorMeta) im;
				Color color = sItemMeta.getColor();
				itemMeta = new HLeatherArmorMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						new HColor(color.getRed(), color.getGreen(), color.getBlue()));
				...
				if (sItemMeta.hasCustomEffects()) {
					for (PotionEffect pe : sItemMeta.getCustomEffects()) {
						potionEffects.add(new HPotionEffect(pe.getType().getName(), pe.getAmplifier(), pe.getDuration(),
								pe.isAmbient()));
					}
				}
				itemMeta = new HPotionMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						potionEffects, potionData);
			} else if (im instanceof SkullMeta) {
				SkullMeta sItemMeta = (SkullMeta) im;
				itemMeta = new HSkullMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						sItemMeta.getOwner());
			} else if (im instanceof MapMeta) {
				MapMeta sItemMeta = (MapMeta) im;
				itemMeta = new HMapMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						sItemMeta.isScaling());
				...
				if (dyeColor != null)
					baseColor = sItemMeta.getBaseColor().toString();
				...
				itemMeta = new HBannerMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						baseColor, patterns);
			} else if (im instanceof SpawnEggMeta) {
				SpawnEggMeta sItemMeta = (SpawnEggMeta) im;
				itemMeta = new HSpawnEggMeta(displayName, lore, enchantments, itemFlags, unbreakable, repairCost,
						sItemMeta.getSpawnedType().name());
			} else {
		...
		if (hItemStack.getItemMeta() != null) {
			...
			if (hItemMeta instanceof HEnchantmentStorageMeta) {
				...
				for (HPotionEffect spe : sItemMeta.getPotionEffects()) {
					PotionEffect pe = new PotionEffect(PotionEffectType.getByName(spe.getType()), spe.getDuration(),
							spe.getAmplifier(), spe.isAmbient());
					pm.addCustomEffect(pe, true);
				}
				HPotionData pd = sItemMeta.getPotionData();
				pm.setBasePotionData(
						new PotionData(PotionType.valueOf(pd.getPotionType()), pd.isExtended(), pd.isUpgraded()));
				...
				for (HPattern hp : sItemMeta.getPatterns()) {
					bm.addPattern(
							new Pattern(DyeColor.valueOf(hp.getDyeColor()), PatternType.valueOf(hp.getPatternType())));
				}
