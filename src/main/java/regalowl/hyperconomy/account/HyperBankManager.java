...
public class HyperBankManager implements HyperEventListener {
	...
	public void handleHyperEvent(HyperEvent event) {
		if (event instanceof DataLoadEvent) {
			...
			if (!(devent.loadType == DataLoadType.PLAYER))
				return;
	...
	private void loadData() {
		...
		while (bankData.next()) {
			HyperBank hBank = new HyperBank(hc, bankData.getString("NAME"), bankData.getDouble("BALANCE"),
					bankData.getString("OWNERS"), bankData.getString("MEMBERS"));
