...
public class HyperPlayerManager implements HyperEventListener {
	...
	public void handleHyperEvent(HyperEvent event) {
		if (event instanceof DataLoadEvent) {
				...
				hc.getMC().runTask(new Runnable() {
					public void run() {
						if (!hc.getDataManager().accountExists(defaultServerShopAccount)) {
							...
							defaultAccount.setBalance(
									hc.getConf().getDouble("shop.default-server-shop-account-initial-balance"));
							defaultAccount.setUUID(UUID.randomUUID().toString());
						}
						hc.getHyperEventHandler()
								.fireEventFromAsyncThread(new DataLoadEvent(DataLoadType.DEFAULT_ACCOUNT));
			...
			try {
				...
				hp.validate();
				if (hp.getName().equalsIgnoreCase(config.getString("shop.default-server-shop-account")))
					hc.getMC().kickPlayer(hevent.getHyperPlayer(), hc.getLanguageFile().get("CANT_USE_ACCOUNT"));
			} catch (Exception e) {
	...
	private void loadData() {
		...
		while (playerData.next()) {
			HLocation sl = new HLocation(playerData.getString("WORLD"), playerData.getDouble("X"),
					playerData.getDouble("Y"), playerData.getDouble("Z"));
			HyperPlayer hplayer = new HyperPlayer(hc, playerData.getString("NAME"), playerData.getString("UUID"),
					playerData.getString("ECONOMY"), playerData.getDouble("BALANCE"), sl, playerData.getString("HASH"),
					playerData.getString("SALT"));
			...
			public void run() {
				...
				for (String p : hc.getMC().getOnlinePlayerNames()) {
					...
					if (!hyperPlayers.containsKey(p.toLowerCase())) {
						...
						if (hp.getUUIDString() != null)
							uuidIndex.put(hp.getUUIDString(), p.toLowerCase());
					} else {
						hp = hyperPlayers.get(p.toLowerCase());
					}
					if (p.equalsIgnoreCase(config.getString("shop.default-server-shop-account")))
						hp.kickPlayer(hc.getLanguageFile().get("CANT_USE_ACCOUNT"));
				}
	...
	/**
	 * @param player
	 * @return The HyperPlayer with the specified name. If the HyperPlayer doesn't
	 *         exist it will be created. This method should only return null if the
	 *         given name is null.
	 */
	...
	public HyperPlayer addPlayer(String player) {
		...
		if (!playersLoaded) {
			...
			hc.getDebugMode().ayncDebugConsoleMessage(
					"addPlayer() called before players loaded for: " + player + "; trace: " + trace);
			return null;
		}
		String playerName = player.toLowerCase();
