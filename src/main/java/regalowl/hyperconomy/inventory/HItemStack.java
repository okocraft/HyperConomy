...
public class HItemStack {
	...
	public HItemStack(HyperConomy hc, HItemMeta itemMeta, String material, short durability, byte data, int amount,
	...
	public HItemStack(String serialized) {
		...
		try {
			...
			this.itemMeta = HItemMetaFactory.generate(HItemMetaType.fromString(data.get("metaType")),
					data.get("itemMetaData"));
	...
	public boolean considerDamage() {
		boolean ignoreDamage = hc.getConf()
				.getBoolean("enable-feature.treat-damaged-items-as-equals-to-undamaged-ones");
		if (!ignoreDamage)
			return true;
		if (maxDurability > 0)
			return false;
	...
	public double getDurabilityPercent() {
		return (isDurable()) ? (1.0 - ((double) durability / (double) maxDurability)) : 1.0;
	}

	public boolean isDurable() {
		return (maxDurability > 0) ? true : false;
	}

	public boolean isDamaged() {
		return (isDurable() && durability > 0) ? true : false;
	}

	public byte getComparisonData() {
		return (isDamaged()) ? 0 : data;
	}

	public short getComparisonDurability() {
		return (isDamaged()) ? 0 : durability;
	}
	...
	public boolean canEnchantItem() {
		if (material.equalsIgnoreCase("AIR"))
			return false;
		if (material.equalsIgnoreCase("BOOK"))
			return true;
	...
	public boolean canAcceptEnchantment(HEnchantment e) {
		...
		if (itemMeta != null) {
			for (HEnchantment en : itemMeta.getEnchantments()) {
				if (hc.getMC().conflictsWith(e, en))
					return false;
			}
		}
	...
	public double addEnchantment(HEnchantment e) {
		if (itemMeta == null)
			itemMeta = new HItemMeta("", new ArrayList<String>(), new ArrayList<HEnchantment>(),
					new ArrayList<HItemFlag>(), false, 0);
		if (itemMeta.containsEnchantment(e))
			return 0;
