...
public class HInventory {
	...
	public boolean isPlayerInventory() {
		if (inventoryType.equals(HInventoryType.PLAYER))
			return true;
		return false;
	}
	...
	public void updateLore(int slot) {
		...
		if (is.isBlank() || is.getItemMeta() == null)
			return;
	...
	public void add(int addAmount, HItemStack addStack) {
		...
		if (addAmount != 0)
			hc.gSDL().getErrorWriter().writeError("HInventory add() failure; " + addAmount + " remaining.");
		// updateInventory();
	}
	...
	public double remove(int removeAmount, HItemStack removeStack) {
		...
		if (removeAmount != 0)
			hc.gSDL().getErrorWriter().writeError(
					"HInventory remove() failure.  Items not successfully removed; amount = '" + removeAmount + "'");
		// updateInventory();
