	...
	public CommandData onCommand(CommandData data) {
		...
		if (args.length == 0) {
			...
			if (args[0].equalsIgnoreCase("pd")) {// prints HC serialized version of ItemStack in hand
				...
				return data;
			} else if (args[0].equalsIgnoreCase("pdf")) {// prints HC serialized version of ItemStack in hand to file
															// named serialized_item.txt
				stack = hp.getItemInHand();
				...
				return data;
			} else if (args[0].equalsIgnoreCase("comp")) {// compares specified HyperObject ItemStack with held item and
															// displays data
				if (args.length == 2) {
