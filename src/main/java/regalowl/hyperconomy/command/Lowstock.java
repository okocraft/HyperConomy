...
public class Lowstock extends BaseCommand {
	...
	public CommandData onCommand(CommandData data) {
		...
		for (int i = startIndex; i < endIndex; i++) {
			...
			if (to.isShopObject()) {
				data.addResponse("&f" + to.getDisplayName() + ": &a" + CommonFunctions.twoDecimals(to.getStock())
						+ " &f(&e" + to.getShopObjectStatus().toString() + "&f)");
			} else {
				data.addResponse(
						"&f" + to.getDisplayName() + "&f: " + "&b" + CommonFunctions.twoDecimals(to.getStock()));
			}
		}
		return data;
	}
