...
public class Audit extends BaseCommand implements HyperCommand {
					...
					hc.getMC().runTask(new Runnable() {
						public void run() {
							if (hp == null) {
								...
								hc.getMC().logInfo(
										L.f(L.get("AUDIT_THEORETICAL1"), CommonFunctions.twoDecimals(logbalance)));
								hc.getMC().logInfo(
										L.f(L.get("AUDIT_THEORETICAL2"), CommonFunctions.twoDecimals(auditbalance)));
								hc.getMC().logInfo(L.get("LINE_BREAK"));
							} else {
	 ...
	 * @param account
	 * @param type
	 * @return returns the theoretical amount of money an account should have after
	 *         all logged (buy or sell individually) transactions in the
	 *         hyperconomy_log
	 */
	private Double getHyperLogTotal(String account, String type) {
		...
		if (type.equalsIgnoreCase("sale")) {
			query = "SELECT SUM(MONEY) AS total FROM hyperconomy_log WHERE CUSTOMER = '" + account
					+ "' AND ACTION = 'sale'";
	 ...
	 * @param account
	 * @return returns the theoretical amount of money an account should have after
	 *         all logged transactions in the hyperconomy_audit_log
	 */
	private Double getAuditLogTotal(String account) {
		SQLRead sr = hc.getSQLRead();
		QueryResult result = sr
				.select("SELECT * FROM hyperconomy_audit_log WHERE ACCOUNT = '" + account + "' ORDER BY TIME ASC");
		double tBalance = 0.0;
