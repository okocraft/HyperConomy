...
public class BasicShopTradeObject extends BasicTradeObject implements TradeObject {
	...
	public BasicShopTradeObject(HyperConomy hc, String playerShop, TradeObject ho, double stock, double buyPrice,
	...
	public void setParentTradeObject(String to) {
		WriteStatement ws = new WriteStatement(
				"UPDATE hyperconomy_shop_objects SET HYPEROBJECT=? WHERE SHOP=? AND HYPEROBJECT=?",
				hc.getSimpleDataLib());
		ws.addParameter(to);
	...
	public void setShopObjectShop(String playerShop) {
		WriteStatement ws = new WriteStatement(
				"UPDATE hyperconomy_shop_objects SET SHOP=? WHERE SHOP=? AND HYPEROBJECT=?", hc.getSimpleDataLib());
		ws.addParameter(playerShop);
	...
	public void setStock(double stock) {
		if (useEconomyStock) {
			...
			this.stock = stock;
			WriteStatement ws = new WriteStatement(
					"UPDATE hyperconomy_shop_objects SET QUANTITY=? WHERE SHOP=? AND HYPEROBJECT=?",
					hc.getSimpleDataLib());
			...
			hc.getHyperEventHandler()
					.fireEvent(new TradeObjectModificationEvent(this, TradeObjectModificationType.STOCK));
		}
	}
	...
	public void setShopObjectBuyPrice(double buyPrice) {
		this.buyPrice = buyPrice;
		WriteStatement ws = new WriteStatement(
				"UPDATE hyperconomy_shop_objects SET BUY_PRICE=? WHERE SHOP=? AND HYPEROBJECT=?",
				hc.getSimpleDataLib());
		...
		sw.addToQueue(ws);
		hc.getHyperEventHandler()
				.fireEvent(new TradeObjectModificationEvent(this, TradeObjectModificationType.BUY_PRICE));
	}
	...
	public void setShopObjectSellPrice(double sellPrice) {
		this.sellPrice = sellPrice;
		WriteStatement ws = new WriteStatement(
				"UPDATE hyperconomy_shop_objects SET SELL_PRICE=? WHERE SHOP=? AND HYPEROBJECT=?",
				hc.getSimpleDataLib());
		...
		sw.addToQueue(ws);
		hc.getHyperEventHandler()
				.fireEvent(new TradeObjectModificationEvent(this, TradeObjectModificationType.SELL_PRICE));
	}
	...
	public void setShopObjectMaxStock(int maxStock) {
		this.maxStock = maxStock;
		WriteStatement ws = new WriteStatement(
				"UPDATE hyperconomy_shop_objects SET MAX_STOCK=? WHERE SHOP=? AND HYPEROBJECT=?",
				hc.getSimpleDataLib());
		...
		sw.addToQueue(ws);
		hc.getHyperEventHandler()
				.fireEvent(new TradeObjectModificationEvent(this, TradeObjectModificationType.MAX_STOCK));
	}
	...
	public void setShopObjectStatus(TradeObjectStatus status) {
		this.status = status;
		WriteStatement ws = new WriteStatement(
				"UPDATE hyperconomy_shop_objects SET STATUS=? WHERE SHOP=? AND HYPEROBJECT=?", hc.getSimpleDataLib());
	...
	// The following methods don't apply to PlayerShop objects and have been
	// overridden to prevent database changes.
	@Override
	...
	// The following methods don't apply to PlayerShop objects and have been
	// overridden to forward the request to the PlayerShop object's parent object.
	@Override
