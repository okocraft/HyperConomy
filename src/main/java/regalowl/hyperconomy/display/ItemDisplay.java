...
public class ItemDisplay {
	...
	public void makeDisplay() {
		...
		this.item = hc.getMC().dropItemDisplay(new HLocation(l.getWorld(), l.getX(), l.getY() + 1, l.getZ()),
				dropstack);
		active = true;
	}
	 ...
	 *
	 * @param droppedItem
	 * @return true if the item drop event shop be blocked to prevent item stacking,
	 *         false if not
	 */
	public boolean blockItemDrop(HItem droppedItem) {
		...
		if (!displayStack.equals(droppedItem.getItem()))
			return false;
		if (!l.getWorld().equals(getWorld()))
			return false;
		if (Math.abs(dl.getX() - l.getX()) > 10)
			return false;
		if (Math.abs(dl.getZ() - l.getZ()) > 10)
			return false;
		if (Math.abs(dl.getY() - l.getY()) > 30)
			return false;
		hc.getMC().zeroVelocity(droppedItem);
		if (hc.getMC().getFirstNonAirBlockInColumn(dl).getLocation().getY() > (l.getY() + 10))
			return false;
		return true;
	}
