...
public class PlayerShop implements Shop {
	...
	public PlayerShop(HyperConomy hc, String name, String economy, HyperAccount owner, String message, HLocation p1,
			HLocation p2, String banned_objects, String allowed_players, boolean useEconomyStock) {
		this.hc = hc;
	...
	private void loadPlayerShopObjects() {
		...
		BasicStatement statement = new BasicStatement("SELECT * FROM hyperconomy_shop_objects WHERE SHOP = ?",
				hc.getSimpleDataLib());
		...
		while (result.next()) {
			...
			if (ho.getType() == TradeObjectType.ITEM && ho.isCompositeObject()) {
				TradeObject pso = new CompositeShopTradeItem(hc, name, (CompositeTradeItem) ho, stock, buyPrice,
						sellPrice, maxStock, status, useEconomyStock);
				shopContents.put(ho.getName(), pso);
			} else if (ho.getType() == TradeObjectType.ENCHANTMENT) {
				TradeObject pso = new ShopTradeEnchant(hc, name, ho, stock, buyPrice, sellPrice, maxStock, status,
						useEconomyStock);
				shopContents.put(ho.getName(), pso);
			} else {
				BasicShopTradeObject pso = new BasicShopTradeObject(hc, name, ho, stock, buyPrice, sellPrice, maxStock,
						status, useEconomyStock);
				shopContents.put(ho.getName(), pso);
			}
		}
		result.close();
	...
	public boolean isTradeable(TradeObject ho) {
		if (!isBanned(ho.getName())) {
			if (ho.isShopObject()) {
				if (ho.getShopObjectStatus() == TradeObjectStatus.NONE) {
					return false;
				}
				return true;
			} else {
	...
	public TradeObject getPlayerShopObject(TradeObject hyperObject) {
		...
		WriteStatement ws = new WriteStatement(
				"INSERT INTO hyperconomy_shop_objects (SHOP, HYPEROBJECT, QUANTITY, BUY_PRICE, SELL_PRICE, MAX_STOCK, STATUS) VALUES (?,?,?,?,?,?,?)",
				hc.getSimpleDataLib());
		...
		if (hyperObject.getType() == TradeObjectType.ITEM && hyperObject.isCompositeObject()) {
			CompositeShopTradeItem pso = new CompositeShopTradeItem(hc, name, (CompositeTradeItem) hyperObject, 0.0,
					0.0, 0.0, 100000, TradeObjectStatus.NONE, useEconomyStock);
			...
			TradeObject pso = new ShopTradeEnchant(hc, name, hyperObject, 0.0, 0.0, 0.0, 100000, TradeObjectStatus.NONE,
					useEconomyStock);
			shopContents.put(hyperObject.getName(), pso);
			hc.getSQLWrite().addToQueue(ws);
			return pso;
		} else {
			TradeObject pso = new BasicShopTradeObject(hc, name, (BasicTradeObject) hyperObject, 0.0, 0.0, 0.0, 100000,
	...
	public void updateHyperObject(TradeObject ho) {
		if (!ho.isShopObject())
			return;
		if (!ho.getShopObjectShop().equals(this))
			return;
		if (!shopContents.containsKey(ho.getName()))
			return;
