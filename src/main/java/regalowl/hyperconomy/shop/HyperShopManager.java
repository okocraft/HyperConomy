...
public class HyperShopManager implements HyperEventListener {
	...
	public void handleHyperEvent(HyperEvent event) {
		if (event instanceof DataLoadEvent) {
			...
			if (!(devent.loadType == DataLoadType.DEFAULT_ACCOUNT))
				return;
	...
	private void loadData() {
		...
		while (shopData.next()) {
			...
			if (type.equalsIgnoreCase("server")) {
				...
				HLocation p1 = new HLocation(shopData.getString("WORLD"), shopData.getInt("P1X"),
						shopData.getInt("P1Y"), shopData.getInt("P1Z"));
				HLocation p2 = new HLocation(shopData.getString("WORLD"), shopData.getInt("P2X"),
						shopData.getInt("P2Y"), shopData.getInt("P2Z"));
				Shop shop = new ServerShop(hc, name, shopData.getString("ECONOMY"), owner,
				...
				if (!config.getBoolean("enable-feature.player-shops")) {
					continue;
				}
				String name = shopData.getString("NAME");
				HLocation p1 = new HLocation(shopData.getString("WORLD"), shopData.getInt("P1X"),
				...
				HLocation p2 = new HLocation(shopData.getString("WORLD"), shopData.getInt("P2X"),
						shopData.getInt("P2Y"), shopData.getInt("P2Z"));
				Shop shop = new PlayerShop(hc, name, shopData.getString("ECONOMY"), owner,
						shopData.getString("MESSAGE"), p1, p2, shopData.getString("BANNED_OBJECTS"),
						shopData.getString("ALLOWED_PLAYERS"),
						(shopData.getString("USE_ECONOMY_STOCK").equalsIgnoreCase("1")) ? true : false);
				shops.put(name, shop);
			} else if (type.equalsIgnoreCase("global")) {
				if (useShops) {
					continue;
				}
				Shop shop = new GlobalShop(hc, "GlobalShop", "default",
						hc.getHyperPlayerManager().getDefaultServerShopAccount(), shopData.getString("BANNED_OBJECTS"));
		...
		if (!useShops && shops.size() == 0) {
			Shop shop = new GlobalShop(hc, "GlobalShop", "default",
					hc.getHyperPlayerManager().getDefaultServerShopAccount());
			shops.put("GlobalShop", shop);
		}
